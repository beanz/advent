...
pub fn readPacket(alloc: std.mem.Allocator, bits: []const u1, i: *usize) anyerror![2]usize {
    const ver: u3 = readBits(u3, bits, i);
    const kind: u3 = readBits(u3, bits, i);
    if (kind == 4) {
        var n: usize = 0;
        while (true) {
            const a = readBits(u5, bits, i);
    ...
    defer argsL.deinit();
    const lti = readBits(u1, bits, i);
    if (lti == 0) { // 15-bit
        const l = readBits(u15, bits, i);
        const end = i.* + l;
        while (i.* < end) {
            const sub = try readPacket(alloc, bits, i);
            vs += sub[0];
            try argsL.append(sub[1]);
        }
    } else {
        const l = readBits(u11, bits, i);
        var j: usize = 0;
        while (j < l) : (j += 1) {
            const sub = try readPacket(alloc, bits, i);
            vs += sub[0];
...
test "part1" {
    const t1a = try parts(aoc.talloc, test1afile);
    try aoc.assertEq(@as(usize, 6), t1a[0]);
    const t1b = try parts(aoc.talloc, test1bfile);
    try aoc.assertEq(@as(usize, 9), t1b[0]);
    const t1c = try parts(aoc.talloc, test1cfile);
    try aoc.assertEq(@as(usize, 14), t1c[0]);
    const t1d = try parts(aoc.talloc, test1dfile);
    try aoc.assertEq(@as(usize, 16), t1d[0]);
    const t1e = try parts(aoc.talloc, test1efile);
    try aoc.assertEq(@as(usize, 12), t1e[0]);
    const t1f = try parts(aoc.talloc, test1ffile);
    try aoc.assertEq(@as(usize, 23), t1f[0]);
    const t1g = try parts(aoc.talloc, test1gfile);
    try aoc.assertEq(@as(usize, 31), t1g[0]);
    const t2a = try parts(aoc.talloc, test2afile);
    try aoc.assertEq(@as(usize, 14), t2a[0]);
    const t2b = try parts(aoc.talloc, test2bfile);
    try aoc.assertEq(@as(usize, 8), t2b[0]);
    const t2c = try parts(aoc.talloc, test2cfile);
    try aoc.assertEq(@as(usize, 15), t2c[0]);
    const t2d = try parts(aoc.talloc, test2dfile);
    try aoc.assertEq(@as(usize, 11), t2d[0]);
    const t2e = try parts(aoc.talloc, test2efile);
    try aoc.assertEq(@as(usize, 13), t2e[0]);
    const t2f = try parts(aoc.talloc, test2ffile);
    try aoc.assertEq(@as(usize, 19), t2f[0]);
    const t2g = try parts(aoc.talloc, test2gfile);
    try aoc.assertEq(@as(usize, 16), t2g[0]);
    const t2h = try parts(aoc.talloc, test2hfile);
...
test "part2" {
    const t1a = try parts(aoc.talloc, "D2FE28\n");
    try aoc.assertEq(@as(usize, 2021), t1a[1]);
    const t1b = try parts(aoc.talloc, "38006F45291200\n");
    try aoc.assertEq(@as(usize, 1), t1b[1]);
    const t1c = try parts(aoc.talloc, "EE00D40C823060\n");
    try aoc.assertEq(@as(usize, 3), t1c[1]);
    const t1d = try parts(aoc.talloc, "8A004A801A8002F478\n");
    try aoc.assertEq(@as(usize, 15), t1d[1]);
    const t1e = try parts(aoc.talloc, "620080001611562C8802118E34\n");
    try aoc.assertEq(@as(usize, 46), t1e[1]);
    const t1f = try parts(aoc.talloc, "C0015000016115A2E0802F182340\n");
    try aoc.assertEq(@as(usize, 46), t1f[1]);
    const t1g = try parts(aoc.talloc, "A0016C880162017C3686B18A3D4780\n");
    try aoc.assertEq(@as(usize, 54), t1g[1]);
    const t2a = try parts(aoc.talloc, "C200B40A82\n");
    try aoc.assertEq(@as(usize, 3), t2a[1]);
    const t2b = try parts(aoc.talloc, "04005AC33890\n");
    try aoc.assertEq(@as(usize, 54), t2b[1]);
    const t2c = try parts(aoc.talloc, "880086C3E88112\n");
    try aoc.assertEq(@as(usize, 7), t2c[1]);
    const t2d = try parts(aoc.talloc, "CE00C43D881120\n");
    try aoc.assertEq(@as(usize, 9), t2d[1]);
    const t2e = try parts(aoc.talloc, "D8005AC2A8F0\n");
    try aoc.assertEq(@as(usize, 1), t2e[1]);
    const t2f = try parts(aoc.talloc, "F600BC2D8F\n");
    try aoc.assertEq(@as(usize, 0), t2f[1]);
    const t2g = try parts(aoc.talloc, "9C005AC2F8F0\n");
    try aoc.assertEq(@as(usize, 0), t2g[1]);
    const t2h = try parts(aoc.talloc, "9C0141080250320F1802104A08\n");
