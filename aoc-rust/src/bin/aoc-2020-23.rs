fn parts(inp: &[u8]) -> (usize, usize) {
    let mut cups: [usize; 9] = [0; 9];
    for i in 0..8 {
        let c = (inp[i] & 0xf) as usize;
        let n = (inp[i + 1] & 0xf) as usize;
        cups[c - 1] = n - 1;
    }
    let mut cur = (inp[0] & 0xf) as usize - 1;
    ...
    for _ in 0..100 {
        ...
        let new_next = cups[new];
        cups[r3] = new_next;
        cups[new] = r1;
        cur = next;
    }
    let p1 = part1(&cups);
    let mut cups: [usize; 1_000_000] = [0; 1_000_000];
    for i in 0..8 {
        let c = (inp[i] & 0xf) as usize;
        let n = (inp[i + 1] & 0xf) as usize;
        cups[c - 1] = n - 1;
