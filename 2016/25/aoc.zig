...
const Op = enum { CpyReg, Load, Inc, Dec, Jnz, Jmp, Nop, Out };
...
fn parts(inp: []const u8) anyerror![2]usize {
    ...
    while (i < inp.len) {
        switch (inp[i]) {
            ...
            'o' => {
                try b.append(Inst{ .op = Op.Out, .lit = 0, .reg = inp[i + 4] - 'a', .out = 0 });
                i += 6;
            },
            'j' => {
                const v = inp[i + 4];
                ...
                if (v == '0') {
                    try b.append(Inst{ .op = Op.Nop, .lit = o, .reg = 0, .out = 0 });
                } else if (v == '1') {
    ...
    const prog = b.slice();
    const n: usize = @as(usize, @intCast(prog[1].lit * prog[2].lit));
    return [2]usize{ 0b101010101010 - n, 0 };
}
